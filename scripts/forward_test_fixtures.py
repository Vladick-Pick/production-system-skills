...
FIXTURES = {
    "resolve-model-element": {
        "required": [
            ...
            "тот же показатель",
            "финансовый показатель использует общий контракт показателя",
            "три независимые оси",
            "привязку показателя к фактам",
            "требование показателя",
    ...
    "maintain-production-system": {
        "required": [
            ...
            "новый показатель",
            "финансовый показатель использует общий контракт показателя",
            "трём независимым осям",
            "режим миграции v0.3 → v0.4",
            "requires_input",
            "новая схема позиции не должна молча менять",
        ],
    },
    "audit-production-system": {
        "required": [
            ...
            "базовую версию",
            "нормы наблюдаемости",
            "универсальному допуску показателя",
            "финансовый показатель использует общий контракт показателя",
            "три независимые оси",
            "для v0.4 проверить точные 37 листов",
            "две основные привязки",
            "условий назначений",
        ],
    },
}
...
def main() -> int:
    ...
    failed_scripts = [
        ...
        for script in (
            ...
            "test_template_builder_v0_4.py",
            "test_migration_v0_3_to_v0_4.py",
