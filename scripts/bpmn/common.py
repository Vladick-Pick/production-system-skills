...
def all_nodes(ir: dict[str, Any]) -> list[dict[str, Any]]:
    return list(ir.get("events", [])) + list(ir.get("actions", [])) + list(ir.get("gateways", []))
...
def validate_ir(ir: dict[str, Any], expected_version_id: str | None = None) -> list[ProjectionIssue]:
    ...
    for section in ("events", "actions", "gateways"):
        if not isinstance(ir.get(section, []), list):
    ...
    for index, action in enumerate(ir.get("actions", [])):
    ...
    for index, gateway in enumerate(ir.get("gateways", [])):
