...
По умолчанию одна исходная книга становится одним отдельным черновиком миграции v0.2 и сохраняет собственный `model_id`. `Staging` допустимо только как внутреннее техническое имя состояния; в названии книги и сообщениях человеку использовать «черновик миграции». Две рабочие книги не объединяются автоматически. Если они описывают один бизнес и должны стать одной книгой, это отдельное решение о границе, конфликтах ID, версиях и источниках; до него агент ведёт два независимых досье.
...
|---|---|---|
| `migration-assessment` | читать источник, экспорт, структуру и данные; строить досье; задавать один вопрос | read-only досье и список блокеров, без записей в черновик миграции |
...
| `migration-write` | после подтверждения показанного пакета записать один ограниченный пакет в черновик миграции | одна проверенная migration transaction |
...
Смена физической схемы не является причиной заново согласовывать смысл каждой строки. При сохранённых stable ID, референте, определении и связях строка остаётся `copy`. Неоднозначность одной строки не блокирует перенос независимых строк: агент переносит всё доказуемое и явно оставляет блокер в досье.
...
В target `source_locator` остаётся точной проверяемой ссылкой, но видимая подпись должна быть понятна человеку: `Исходная v0.1 → <лист> → <строка или диапазон>`. Служебную строку `gsheet:<id>#<range>` не показывать как единственное содержимое ячейки.
...
Не задавать этот список анкетой. Сначала прочитать всю доступную книгу, сгруппировать одинаковые неизвестные и задать один вопрос с наибольшим downstream-влиянием. После ответа обновить досье, но не писать в черновик миграции.
...
### E. Разбить перенос на ограниченные пакеты

Большую модель не нужно переносить одной гигантской transaction. Она переносится как `bounded batches` — ограниченные пакеты с ясной границей. Один migration run объединяется общими source fingerprint, target `model_id`, working version и migration plan, но каждый пакет имеет собственные `package_id`, внутренний hash, confirmation, decision и transaction. Исходная цель «перенести книгу» остаётся активной между пакетами.
...
Пакет не должен содержать FK на ещё не созданную сущность. Нерешённый семантический blocker не закрывается placeholder ID. Допустимо подготовить черновик миграции с конфликтами, но такая версия не становится `принято` или `действует`.

### F. Записать и проверить каждый batch

В `migration-write` создать или использовать отдельный черновик миграции v0.2. Перед каждой записью показать полный пакет, before/after, source locators, все operations, decision/change rows, preconditions и recovery. `package_hash` вычисляется и хранится агентом; пользователь не обязан читать или перепечатывать его. После простого явного человеческого подтверждения записи последнего показанного пакета записать одну transaction, выполнить read-back и checkpoint.

После каждого пакета показать четыре строки статуса:

- перенесено сейчас;
- перенесено всего по исходным авторским листам;
- осталось и почему;
- общий перенос `не завершён` либо `завершён`.

Нельзя завершать задачу на основании созданного каркаса, настроек версии или одного семантического блока. Статус `complete` допустим только после reconciliation всех исходных авторских строк: каждая строка перенесена, сознательно архивирована/отброшена с причиной либо перечислена как unresolved.
...
source:
  ...
  backup_locator: "..."
target:
  spreadsheet_id: "..."      # null до разрешённого черновика миграции
  model_id: "..."
  schema_version: "0.2"
  working_version_id: "..."
editor:
...
- для мигрируемой рабочей модели создан отдельный черновик миграции и подтверждено право его изменять;
...
Для конкретной рабочей модели rollback означает отклонить черновик миграции и продолжить использовать сохранённую v0.1-копию. Публичный шаблон уже выпущен как v0.2; его последующий откат оформляется новым релизом, а manifest и бинарные снимки не переписываются задним числом.
