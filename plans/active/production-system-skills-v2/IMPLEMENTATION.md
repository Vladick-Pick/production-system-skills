...
Не выполнять внутри локальных пакетов: изменение публичного Google Sheet, рабочих моделей, push/PR, deployment Camunda и публикацию закрытых Google Docs. Эти действия имеют отдельные ворота.

## 2. Внешние ворота

| Ворота | Что требуется | Что блокирует |
|---|---|---|
| G-1 | Проверить название, доступ, актуальность и владельца двух нормативных Google Docs | Полную синхронизацию языка и корпоративную BPMN-спецификацию |
| G-2 | Отдельное разрешение на staging Google Sheet и затем отдельное разрешение на публикацию | WP-5 и WP-10 соответственно |
...
2. рядом расположен `*_selector` вида `читаемое имя — контекст ⟦stable_id⟧`;
...
**Done:** staging/XLSX совпадают; публичный v0.1 не изменён. **Rollback:** удалить staging.
...
**Зависимости:** WP-8 PASS, WP-9 PASS, G-2 на публикацию. **Owned files:** весь согласованный release surface.
...
**Done:** один публичный шаблон, один manifest, четыре синхронных скилла, PASS-evidence и rollback v0.1. Родительский план архивируется только после подтверждения владельца.
...
- template — публичный v0.1 и его XLSX неизменны до release gate;
- staging — удаляется без влияния на canonical source;
...
10. Publication/rollout выполнены только после G-2/G-3.
