...
- `template-schema-v0.4.json` — overlay release candidate: пять реестров карты для показателей, привязок, требований, экономических правил и условий назначений, до итоговых 37 листов;
...
- `scripts/build_template_v0_4.py` — композиционный builder локального release candidate v0.4;
...
- `migrations/v0.3-to-v0.4.md` — управляемая миграция: вся существующая модель сохраняется, legacy-показатели и нормативы требуют смыслового разрешения, а экономика не восстанавливается по догадке.
...
V0.4 до отдельной визуальной приёмки и публикации существует только как локальный release candidate: schema, builder, migration, тесты и review-артефакт. Он не меняет `template-manifest.yaml`, публичную ссылку или канонический XLSX v0.3 молча.
